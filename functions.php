...
function sv_remove_publish_topics_cap() {
    // Get the participant role object.
    $participant_role = get_role( 'bbp_participant' );

    // Check if the role exists.
    if ( ! empty( $participant_role ) ) {
        // Remove the 'publish_topics' capability.
        $participant_role->remove_cap( 'publish_topics' );
    }
}

// Hook the function to 'admin_init' to ensure it runs when the admin interface loads.
add_action( 'admin_init', 'sv_remove_publish_topics_cap' );

/**
 * Optional: Prevent participants from creating new topics in the front-end as well.
 * This function checks the current user's capabilities before allowing them to create a new topic.
 * It's important to have this check, especially if you have forms or other ways for users to create topics
 * on the front-end of your site.  This works in conjunction with the admin_init hook above.
 */
function sv_check_publish_topic_capability() {
    if ( ! current_user_can( 'publish_topics' ) && bbp_is_topic_form() ) {
        // You can use a variety of methods to stop the user.  Here are a few:

        // 1.  Redirect to the forum index page with a message.
        wp_safe_redirect( bbp_get_forum_permalink() );
        exit; // Important:  Always exit after a redirect.

        // 2.  Display an error message and prevent the form from processing.
        // bbp_add_error( 'topic_creation_denied', __( 'Sorry, you are not allowed to create new topics.', 'your-text-domain' ) );
        // bbp_get_template_part( 'form', 'topic' ); //or use another template
        // exit;

        // 3.  Use a WordPress error message.
        // wp_die( __( 'You do not have sufficient permissions to create topics.', 'your-text-domain' ) );
    }
}
// Hook this function to a suitable action that runs before topic creation.
// bbp_is_topic_form() is NOT an action, it's a conditional tag.  Find the action
// that bbPress uses *before* it creates the topic.  This is an example, you might need
// to adjust the hook.  A good place to start looking is within the bbPress plugin
// code, in the functions that handle form submissions.
//
// IMPORTANT:  This hook name ('bbp_before_insert_topic') is an example.  You MUST
//              check the bbPress source code for the correct hook.  This is crucial.
// add_action( 'bbp_before_insert_topic', 'sv_check_publish_topic_capability' );

/**
 * IMPORTANT NOTE:
 * The key to making this work robustly is to:
 * 1. Remove the capability on 'admin_init' (the first function).  This ensures it's removed in the backend.
 * 2. Add a check *before* the topic is created on the front-end (the second function).  This prevents users from bypassing the backend restrictions.
 * 3. **Find the correct bbPress action hook** for where topic data is processed *before* insertion.  The 'bbp_before_insert_topic' used above is an *example* and might not be the correct one.  You'll need to consult the bbPress source code.
 */
