...
function um_notify_admin_on_account_update($user_id, $changes)
{
    ...
    if (isset($ultimate_member_options['admin_email'])) {
        $admin_email_val = $ultimate_member_options['admin_email'];
    }
    $admin_email = $admin_email_val . ',donald@cprogress.co.uk,bradley@cprogress.co.uk';
    ...
    if ($admin_email) {
        ...
        $message_admin = do_shortcode(sprintf('%s', carbon_get_theme_option('user_details_updated_admin_email_message')) . "\r\n\r\n");
        $subject_user = sprintf('[%s] %s', get_bloginfo('name'), carbon_get_theme_option('user_details_updated_client_email_subject'));
        $message_user = do_shortcode(sprintf('%s', carbon_get_theme_option('user_details_updated_client_email_message')) . "\r\n\r\n");
...
function member_directory_email_fields($post_id, $new = false, $button_url = false)
{
    ...
    if ($new == true) {
        ...
        if ($changes_html != '') {
            ...
            $email_html .= $changes_html;
            if ($button_url) {
                $email_html .= '<tr><td colspan="3" style="padding-top: 30px"><div style="padding: 10px 0 50px 0; text-align: center;" data-mce-style="padding: 10px 0 50px 0; text-align: center;"><table style="margin-left: auto; margin-right: auto">
...
function addReadMore($text, $limit = 200, $readMoreText = 'Read More')
{
    ...
    if (strlen($text) > $limit) {
        ...
        // Truncate the text to the last space before the limit
